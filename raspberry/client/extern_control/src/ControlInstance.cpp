...
ControlInstance* ControlInstance::instance_ = 0;
...
ControlInstance* ControlInstance::instance()
{
  if (instance_ == 0)
  {
    instance_ = new ControlInstance();
  }
  return instance_;
}

//--------------------------------------------------------------------
void ControlInstance::setController(UController* u_controller)
{
...
//--------------------------------------------------------------------
void ControlInstance::gpioDeInit()
{
...
//--------------------------------------------------------------------
void ControlInstance::buttonCallback()
{
  debug(CTL_HANDLER, "buttonCallback: Button pressed\n");
}

//--------------------------------------------------------------------
void* ControlInstance::gpioFunction(void* arg)
{
  ControlInstance* ch = (ControlInstance*)arg;
...
//--------------------------------------------------------------------
void ControlInstance::resetUController()
{
...
//--------------------------------------------------------------------
void* ControlInstance::i2cFunction(void* arg)
{
  ControlInstance* ch = (ControlInstance*)arg;
  ...
  // {
  //   debug(WARNING, "ControlInstance::i2cFunction: Could not write data via i2c\n");
  //   return 0;
  // }
  ...
  while (ch->i2c_running_)
  {
    ...
    switch (mode)
    {
      ...
      case OFFSET:
        ...
        break;
      default:
        debug(WARNING, "ControlInstance::i2cFunction: Not knowing mode %d\n", mode);
    }
...
void ControlInstance::writeI2c(ControlInstance* ch, uint8_t reg, const uint8_t* data, int length)
{
  ...
  {
    debug(WARNING, "ControlInstance::writeI2c: %d errors on the i2c line\n", ch->i2c_error_.load());
